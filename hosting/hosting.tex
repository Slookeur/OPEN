...
\\[-0.5cm]
\noindent If you intend to let \github\-pages build your site, then know that \github\ pages are best compatible with version 3.10.x, 
to check the latest \github\ dependency version: \\
...
\comm{Enable or disable the site search}
\comm{Supports true (default) or false}
\var{search\_enabled} \magenta{true}
\var{search:}
  \comm{Split pages into sections that can be searched individually, supports 1 - 6, default: 2}
  \var{heading\_level:} \magenta{2}
  \comm{Maximum amount of previews per search result, default: 3}
  \var{previews:} \magenta{3}
  \comm{Maximum amount of words to display before a matched word in the preview, default: 5}
  \var{preview\_words\_before:} \magenta{5}
  \comm{Maximum amount of words to display after a matched word in the preview, default: 10}
  \var{preview\_words\_after:} \magenta{10}
  \comm{Set the search token separator, default:} /[{\textbackslash}s\textbackslash-/]+/
  \comm{Example: enable support for hyphenated search words}
  \var{tokenizer\_separator:} /[{\textbackslash}s/]+/
  \comm{Display the relative url in search results, supports true (default) or false}
  \var{rel\_url:} \magenta{true}
  \comm{Enable or disable the search button that appears in the bottom right corner of every page, true or false (default)}
  \var{button:} \magenta{true}
  \comm{Focus the search input by pressing `ctrl + focus\_shortcut\_key` (or `cmd + focus\_shortcut\_key` on macOS)}
