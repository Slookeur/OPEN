...
\confb{AC\_CHECK\_PROG}{[\rtt{PUP\_MIME}], [update-mime-database], [yes], [no]}
\confb{AC\_CHECK\_PROG}{[\rtt{PUP\_DESKTOP}], [update-desktop-database], [yes], [no]}
\confb{AC\_CHECK\_PROG}{[\rtt{PUP\_APPSTREAM}], [appstream-util], [yes], [no]}
\end{script}
}}
\vspace{-1cm}
...
\comm{AC_CANONICAL_HOST is needed to access the '\$host_os' variable}
\confa{AC_CANONICAL_HOST}

build_linux=no
build_windows=no
build_mac=no

\comm{Detect the target system using the \$host_os variable}
case "\magenta{\${host_os}}" in
    linux*)
        build_linux=yes
    ...
    darwin*)
        ...
        ;;
    *)
        \confb{AC_MSG_ERROR}{["\magenta{OS \$host_os is not supported"}]}
        ;;
esac

\comm{Pass the conditionals to automake}
\confb{AM_CONDITIONAL}{[\rtt{LINUX}], [test "\magenta{\$build_linux}" = "\magenta{yes}"]}
\confb{AM_CONDITIONAL}{[\rtt{WINDOWS}], [test "\magenta{\$build_windows}" = "\magenta{yes}"]}
\confb{AM_CONDITIONAL}{[\rtt{OSX}], [test "\magenta{\$build_mac}" = "\magenta{yes}"]}

if test "\magenta{\$build_windows}" = "\magenta{yes}"; then
  \confb{AC_CHECK_TOOL}{[\rtt{WINDRES}], [windres], [no]}
  if test "\magenta{\$WINDRES}" = no; then
    \confb{AC_MSG_ERROR}{\cstr{\magenta{*** Could not find an implementation of windres in your PATH.}}}
  fi
fi
\end{script}
}}
...
\dnl{GTK3 for OSX requires the package gtk-mac-integration}
if test "\magenta{\$build_mac}" = "\magenta{yes}"; then
  \confb{PKG_CHECK_MODULES}{\rtt{INTEGRATION}, \cstr{\bftt{gtk-mac-integration}}}
fi
