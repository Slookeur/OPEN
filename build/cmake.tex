...
\begin{script}
\fprompt{~} \bftt{gcc} \dgtt{-D}OPENMP \dgtt{-D}PACKAGE_LOGO=\magenta{"pixmaps/logo.png"}
\end{script}
...
\noindent "\bftt{C\_SOURCES}" is the name of a variable that will contain the list of files with the extension "\texttt{*.c}" that CMake can find recursively in the source directory "\texttt{src}". \\
Similarly "\bftt{F\_SOURCES}" is the name of a variable that will contain the list of files with the extension "\texttt{*.f90}" that CMake can find recursively in the source directory "\texttt{src}".
...
\noindent "\bftt{C\_SRC}" is the name of a variable that will contain the list of files with the extension "\texttt{*.c}" that CMake can find in the source directory "\texttt{src}" only. \\
Similarly "\bftt{F\_SRC}" is the name of a variable that will contain the list of files with the extension "\texttt{*.f90}" that CMake can find in the source directory "\texttt{src}" only. 
\end{itemize}
...
\begin{script}
\confb{add_executable}{\gtt{prog} \varc{C_SOURCES} \varc{F_SOURCES}}
\end{script}
\\[-0.25cm]
\noindent Where "\gtt{prog}" is the project name (see page~\pageref{cmake_pname}), followed by the list of sources, here C and Fortran90 sources files declared previously and listed in the "\varc{C\_SOURCES}" and "\varc{F\_SOURCES}" variables. 
...
\comm{Post installation script}
\confb{install}{\bftt{SCRIPT} post-install.cmake}
\end{script}
}}
