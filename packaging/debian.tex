...
{\footnotesize{
\begin{script}
\uprompt{~/program-1.2.12} ls ../program*
program-1.2.12  \rtt{program-1.2.12.tar.gz}
...
\vspace{-0.25cm}
\begin{scripti}
]\$ git checkout \bftt{upstream}
]\$ \bftt{gbp} \rtt{tag} \btt{upstream/1.2.12}
\end{scripti}
\\[-0.75cm]
...
]\$ \bftt{gbp} \rtt{pristine-tar} \btt{commit} ../program\_1.2.12.orig.tar.gz
\end{scripti}
\\[-0.75cm]
...
\end{scripti}
\\[-1.0cm]
\item You can also use \bftt{gbp}, for \bftt{g}it \bftt{b}uild \bftt{p}ackage, to build the package locally:
\vspace{-0.25cm}
\begin{scripti}
]\$ \bftt{gbp} \rtt{buildpackage}
\end{scripti}
\\[-1.0cm]
...
{\footnotesize{
\begin{scripti}
\uprompt{~} \bftt{gbp} \rtt{clone} git@salsa.debian.org:MyDebianTeam/program.git
\uprompt{~} cd program
\end{scripti}
}}\\[-1.25cm]
...
\vspace{-0.25cm}
\begin{scripti}
\uprompt{~/program} git checkout \bftt{master}
\uprompt{~/program} \bftt{gbp} \rtt{import-orig} \btt{--uscan}
\end{scripti}
\\[-1cm]
This will: 
\begin{itemize}
\item Download the latest sources using the information in the "\texttt{debian/watch}" file.
\item Update the content of the repository using the new sources. 
\item Create the corresponding "\texttt{programl\_VERSION.orig.tar.gz}" archive.
\end{itemize}
