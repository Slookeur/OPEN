...
In the following I use the "{\em{section(s)}}" terminology to refer to parts of the file that might be referred to as "{\em{field(s)}}" in other Debian packaging manuals. 
...
\end{itemize}
Debian offers a tool to handle packaging using the \bftt{git} command:
\begin{script}
\uprompt{~} sudo apt install git-buildpackage quilt
\end{script}\\[-0.5cm]
\noindent The Debian packages "\texttt{git-buildpackage}" and "\texttt{quilt}" provide respectively access to the \bftt{gbp} command that you can use to maintain your package with Git,
and to \bftt{quilt} command that allows to test potential patch(es) associated to your package:
\begin{itemize}
\item Test the patches (optional):
\vspace{-0.25cm}
\begin{scripti}
\uprompt{~/program-1.2.12} QUILT\_PATCHES="debian/patches"
\uprompt{~/program-1.2.12} export QUILT\_PATCHES
\uprompt{~/program-1.2.12} \bftt{quilt} \rtt{push}
\end{scripti}
...
\end{scripti}
\\[-1.25cm]
\item Use \bftt{gbp} to create a new entry in the "\texttt{debian/changelog}" file, using commit history:
\vspace{-0.25cm}
\begin{scripti}
\uprompt{~/program-1.2.12} \bftt{gbp} \rtt{dch} \btt{-R -N 1.2.12-1}
\end{scripti}
\vspace{-1.25cm}
\item When you have prepared a release ready repository, tag it using:
\vspace{-0.25cm}
\begin{scripti}
\uprompt{~/program-1.2.12} \bftt{gbp} \rtt{tag} \btt{upstream/1.2.12}
\end{scripti}
\\[-1.0cm]
...
\item Finally use \bftt{gbp}, for \bftt{g}it \bftt{b}uild \bftt{p}ackage, to build the package:
\vspace{-0.25cm}
\begin{scripti}
...
If you prepared the "{\texttt{debian/watch}}" (see Sec.~\ref{debwatch}) properly then it is possible to update your Salsa repository in a few steps using the data from the official repository (ex: \github): 
\begin{itemize}
\item Download the sources of your Debian package: 
\vspace{-0.25cm}
{\footnotesize{
\begin{scripti}
\uprompt{~} apt-get source program
\uprompt{~} ls
\btt{program-1.2.11}      \rtt{program\_1.2.11-1.debian.tar.xz} 
program\_1.2.11-1.dsc\rtt{  program\_1.2.11.orig.tar.gz}
\end{scripti}
}}\\[-1.25cm]
\item Use the \bftt{uscan} command to build the latest "\texttt{.orig.}" archive by downloading the latest release from the official repository:
\vspace{-0.25cm}
{\footnotesize{
\begin{scripti}
\uprompt{~} cd program-1.2.11
\uprompt{~/program-1.2.11} \bftt{uscan}
...
\uprompt{~/program-1.2.11} cd .. 
\uprompt{~} ls
\btt{program-1.2.11 }                 \btt{   program-1.2.12}
\rtt{program\_1.2.11-1.debian.tar.xz }  \dctt{program\_1.2.12.orig.tar.gz}
program\_1.2.11-1.dsc                \rtt{  program-1.2.12.tar.gz}
\rtt{program\_1.2.11.orig.tar.gz}
\end{scripti}
}}\\[-1.25cm]
\item Clone the Salsa repository, and update its content with the new package:
\vspace{-0.25cm}
{\footnotesize{
\begin{scripti}
\uprompt{~} gbp clone git@salsa.debian.org:MyDebianTeam/program.git
\uprompt{~} cd program
\uprompt{~/program} gbp import-orig ../program-1.2.12.orig.tar.gz
\end{scripti}
}}\\[-1.25cm]
\item Then simply update the repository as detailed in the previous section. 
\end{itemize}
